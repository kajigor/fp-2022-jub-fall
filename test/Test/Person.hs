...
unit_descendants = do
  ...
  thisPerson (Set.elemAt 0 (allDescendants (descendants david allPeople))) @?= christian
  Set.size (allDescendants (descendants christian allPeople)) @?= 3
  thisPerson (Set.elemAt 0 (allDescendants (descendants christian allPeople))) @?= emmeline
  thisPerson (Set.elemAt 1 (allDescendants (descendants christian allPeople))) @?= joseph
  thisPerson (Set.elemAt 2 (allDescendants (descendants christian allPeople))) @?= michael
  Set.size (allDescendants (descendants slobodan allPeople)) @?= 1
  thisPerson (Set.elemAt 0 (allDescendants (descendants slobodan allPeople))) @?= sibi
  Set.size (allDescendants (descendants sibi allPeople)) @?= 3
  thisPerson (Set.elemAt 0 (allDescendants (descendants sibi allPeople))) @?= emmeline
  thisPerson (Set.elemAt 1 (allDescendants (descendants sibi allPeople))) @?= joseph
  thisPerson (Set.elemAt 2 (allDescendants (descendants sibi allPeople))) @?= michael
  thisPerson (descendants emmeline allPeople) @?= emmeline
  Set.size (allDescendants (descendants emmeline allPeople)) @?= 0
  thisPerson (descendants joseph allPeople) @?= joseph
  Set.size (allDescendants (descendants joseph allPeople)) @?= 0
  thisPerson (descendants michael allPeople) @?= michael
  Set.size (allDescendants (descendants michael allPeople)) @?= 0
