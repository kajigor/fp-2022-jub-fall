...
import Expr
import Expr (ArithmeticError(RootOfNegativeNumber, NegativeRoot))
...
unit_genExpr = do
    let results = [Right 13, Right (-42), Right 0.5
                  , Left DivisionByZero, Left LogOfZero, Left LogOfNegativeNumber, Left RootOfNegativeNumber, Left NegativeRoot]
    mapM_ check results
  where
