...
validateShape (Rectangle (PointD x0 y0) (PointD x1 y1)) = and [ x0 /= x1, y0 /= y1 ]

-- Считает периметр фигуры
perimeter :: Shape -> Double
perimeter (Circle _ radius) = pi * radius * 2
...
isPointInShapeOfNormalizedRectangleHelper :: Shape -> PointT -> Bool
isPointInShapeOfNormalizedRectangleHelper (Rectangle (PointD x0 y0) (PointD x1 y1)) (PointD px py) = (x0 < px) && (px < x1) && (y0 < py) && (py < y1)

-- Проверяет, находится ли точка внутри данной фигуры
isPointInShape :: Shape -> PointT -> Bool
isPointInShape (Circle (PointD x y) radius) (PointD px py) = (x - px) ^ 2 + (y - py) ^ 2 < radius ^ 2
isPointInShape (Rectangle (PointD x0 y0) (PointD x1 y1)) point = isPointInShapeOfNormalizedRectangleHelper (normalizeRectangle (Rectangle (PointD x0 y0) (PointD x1 y1))) point
