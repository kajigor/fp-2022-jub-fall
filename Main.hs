...
isSquare :: Shape -> Bool
isSquare (Rectangle (PointD x0 y0) (PointD x1 y1)) = abs (x1 - x0) == abs(y1 - y0) && validateShape (Rectangle (PointD x0 y0) (PointD x1 y1))
isSquare _ = False

-- Передвигает фигуру на x по горизонтали и на y по вертикали
slideShape :: Shape -> PointT -> Shape
slideShape (Circle (PointD x y) radius) (PointD xMove yMove) = Circle (PointD (x + xMove) (y + yMove)) radius
slideShape (Rectangle (PointD x0 y0) (PointD x1 y1)) (PointD xMove yMove) = Rectangle (PointD (x0 + xMove) (y0 + yMove)) (PointD (x1 + xMove) (y1 + yMove))

dist :: PointT -> PointT -> Double
dist (PointD x1 y1) (PointD x2 y2) = sqrt ((x1 - x2)^2 + (y1 - y2)^2)

inInterval :: Double -> Double -> Double -> Bool
inInterval left right point = (min left right < point) && (point < max left right)
...
isPointInShape (Rectangle (PointD x0 y0) (PointD x1 y1)) (PointD x y) = inInterval x0 x1 x && inInterval y0 y1 y
