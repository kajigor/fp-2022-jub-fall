...
validateShape (Rectangle (PointD x0 y0) (PointD x1 y1)) = (x1 - x0) /= 0 && (y1 - y0) /= 0
...
slideShape (Circle (PointD x0 y0) radius) (PointD dx dy) = Circle (PointD (x0 + dx) (y0 + dy)) radius
slideShape (Rectangle (PointD x0 y0) (PointD x1 y1)) (PointD dx dy) = Rectangle (PointD (x0 + dx) (y0 + dy)) (PointD (x1 + dx) (y1 + dy))

-- Считает квадрат расстояния между точками
sqdist :: PointT -> PointT -> Double
sqdist (PointD x0 y0) (PointD x1 y1) = ((x0 - x1)^2 + (y0 - y1)^2)

-- Проверяет, находится ли число в числовом интервале
isBetween :: Double -> Double -> Double -> Bool
isBetween a b x = (min a b) < x && x < (max a b)

-- Проверяет, находится ли точка внутри данной фигуры
isPointInShape :: Shape -> PointT -> Bool
isPointInShape (Circle center radius) pt = (sqdist center pt) < radius^2
isPointInShape (Rectangle (PointD x0 y0) (PointD x1 y1)) (PointD rx ry) = (isBetween x0 x1 rx) && (isBetween y0 y1 ry)
