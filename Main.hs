...
-- Передвигает точку на x по горизонтали и на y по вертикали
slidePoint :: PointT -> PointT -> PointT
slidePoint (PointD x0 y0) (PointD x y) = PointD (x0 + x) (y0 + y)

-- Передвигает фигуру на x по горизонтали и на y по вертикали
slideShape :: Shape -> PointT -> Shape
slideShape (Rectangle p1 p2) p = Rectangle (slidePoint p1 p) (slidePoint p2 p)
slideShape (Circle center radius) p = Circle (slidePoint center p) radius
...
isPointInShape (Rectangle (PointD x0 y0) (PointD x1 y1)) (PointD x y) =
    ((x0 < x && x < x1) || (x1 < x && x < x0)) && ((y0 < y && y < y1) || (y1 < y && y < y0))
isPointInShape (Circle (PointD x0 y0) radius) (PointD x y) = (abs (x0 - x)) ^ 2 + (abs (y0 - y)) ^ 2 < radius ^ 2
