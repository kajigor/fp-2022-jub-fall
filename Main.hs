ii{-# OPTIONS_GHC -Wno-incomplete-patterns #-}
...
isPointInShape (Rectangle (PointD x0 y0) (PointD x1 y1)) (PointD dx dy) = (dx > min x0 x1) && (dx < max x0 x1) && (dy > min y0 y1) && (dy < max y0 y1)
isPointInShape (Circle (PointD x y) radius) (PointD dx dy) = sqrt ((dx - x) ^ 2 + (dy - y) ^ 2) < radius
...
shouldBe x y = x == yi-- Точка на плоскости
data PointT = PointD Double Double
-- Фигуры
data Shape = Circle PointT Double    -- Круг характеризуется координатой центра и радиусом
           | Rectangle PointT PointT -- Прямоугольник со сторонами, параллельными координатным осям,
