...
normalizeRectangle other = other
...
slideShape (Rectangle (PointD x0 y0) (PointD x1 y1)) (PointD xx yy) = Rectangle (PointD (x0 + xx) (y0 + yy)) (PointD (x1 + xx) (y1 + yy))
slideShape (Circle (PointD x y) radius) (PointD xx yy) = Circle (PointD (x + xx) (y + yy)) radius

-- Проверяет, находится ли точка внутри данной фигуры
isPointInShape :: Shape -> PointT -> Bool
isPointInShape (Rectangle (PointD x0 y0) (PointD x1 y1)) (PointD xx yy) = min x0 x1 < xx && max x0 x1 > xx && min y0 y1 < yy && max y0 y1 > yy
isPointInShape (Circle (PointD x y) radius) (PointD xx yy) = sqrt (abs (x - xx) ^ 2 + abs (y - yy) ^ 2) < radius
