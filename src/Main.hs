...
normalizeRectangle x@(Circle _ _) = x
normalizeRectangle (Rectangle (PointD x1 y1) (PointD x2 y2)) = do
  let minx = min x1 x2
  let miny = min y1 y2
  let maxx = max x1 x2
  let maxy = max y1 y2
  Rectangle (PointD minx miny) (PointD maxx maxy)
...
validateShape (Rectangle (PointD x1 y1) (PointD x2 y2)) = do
  let dx = x2 - x1
  let dy = y2 - y1
  dx /= 0 && dy /= 0
...
isSquare (Rectangle (PointD x1 y1) (PointD x2 y2)) = abs (x1 - x2) == abs (y1 - y2)

-- Передвигает фигуру на x по горизонтали и на y по вертикали
slideShape :: Shape -> PointT -> Shape
slideShape (Circle center r) diff = Circle (center <> diff) r
slideShape (Rectangle p1 p2) diff = Rectangle (p1 <> diff) (p2 <> diff)

-- Проверяет, находится ли точка внутри данной фигуры
isPointInShape :: Shape -> PointT -> Bool
isPointInShape (Circle center r) point = sqrDist center point < r * r
  where
    sqrDist (PointD x1 y1) (PointD x2 y2) = do
      let dx = x1 - x2
      let dy = y1 - y2
      dx * dx + dy * dy

isPointInShape rect p = inNormRect (normalizeRectangle rect) p
  where
    inNormRect (Circle _ _) _ = undefined
    inNormRect (Rectangle (PointD x1 y1) (PointD x2 y2)) (PointD x y) = x1 < x && x < x2 && y1 < y && y < y2
