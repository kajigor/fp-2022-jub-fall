...
sumAndMult (AtLeastOne x xs) = let (nextSum, nextMult) = sumAndMult xs in (x + nextSum, x * nextMult)
...
append Empty another = another
append (AtLeastOne x xs) another = AtLeastOne x (append xs another)
