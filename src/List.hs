...
zipWith' :: (a -> b -> c) -> [a] -> [b] -> [c]
zipWith' f xs ys = map f' (zip xs ys) where
  f' (a, b) = f a b
...
triangularNumbers = triangularNumbers' 1 where
  triangularNumbers' n = 0 : map (+n) (triangularNumbers' (n + 1))

-- triangularNumbers :: [Int]
-- triangularNumbers = [(n * (n + 1)) `div` 2 | n <- [0..]]