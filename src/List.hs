module List where
    ...
    sumAndMult (AtLeastOne x xs) = (x + fst(sumAndMult(xs)), x * snd(sumAndMult(xs)))
    ...
    maxNum (AtLeastOne x1 (AtLeastOne x2 xs)) = maxNum(AtLeastOne(if x1 >= x2 then x1 else x2) xs)


    -- Конкатенация двух списков, работает за длину первого списка
    append :: List a -> List a -> List a
    append Empty lst = lst
    append (AtLeastOne x xs) lst = AtLeastOne x (append xs lst)
