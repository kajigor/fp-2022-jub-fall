...
sumAndMult (AtLeastOne x xs) = (x + p1(sumAndMult xs), x * p2(sumAndMult xs))
    where
        p1 :: (Int, Int) -> Int
        p1 (num1, num2) = num1
...
maxNum :: List Int -> Int
maxNum (AtLeastOne x xs) = maximum(x, maxNum xs)
    where
        maximum :: (Int, Int) -> Int
        maximum (num1, num2) = if num1 >= num2 then num1 else num2 
