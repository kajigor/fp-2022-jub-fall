...
sumAndMult list = fold (\x (a, b) -> (a + x, b * x)) (0, 1) list
...
maxNum :: List Int -> Int
maxNum list = fold (\a b -> max a b) (minBound :: Int) list

-- Конкатенация двух списков, работает за длину первого списка
append :: List a -> List a -> List a
append Empty list = list
append (AtLeastOne head tail) list = AtLeastOne head (append tail list)
