...
comparePersons = (compare `on` lastName) <> (compare `on` firstName) <> (compare `on` identification)

comparePets :: Pet -> Pet -> Ordering
comparePets = (comparePersons `on` owner) <> (compare `on` species) <> (compare `on` name)