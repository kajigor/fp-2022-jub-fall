...
ageUp :: Person -> Person
ageUp person = Person (firstName person) (lastName person) (formerLastNames person) (1 + age person) (idNumber person)
...
validatePerson person
  ...
  | age person < 0 = False
  | not (validateIdNumber (age person) (idNumber person)) = False
  | otherwise = True

validateIdNumber :: Int -> (Int, Int) -> Bool
validateIdNumber agePerson idNumberPerson = if agePerson < 14 then (idNumberPerson ==  (0000, 000000)) else True

-- Проверить, что два человека -- тезки.
-- Тезки -- разные люди с одинаковыми именами и фамилиями
namesakes :: Person -> Person -> Bool
namesakes x y
  | firstName x == firstName y && lastName x == lastName y && idNumber x /= idNumber y = True
  | otherwise = False