...
updateLastName person newLastName = if not (lastName person == newLastName) then person {formerLastNames = lastName person : formerLastNames person, lastName = newLastName}
                                    else person
...
validatePerson person = not (null (firstName person)) && not (null (lastName person)) && age person >= 0 && 
                        ((age person < 14 && idNumber person == (0000, 000000)) || (age person >= 14 && not (idNumber person == (0000, 000000))))
  

-- Проверить, что два человека -- тезки.
-- Тезки -- разные люди с одинаковыми именами и фамилиями
namesakes :: Person -> Person -> Bool
namesakes x y = firstName x == firstName y && lastName x == lastName y && idNumber x /= idNumber y 
